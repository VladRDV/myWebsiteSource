	...
	return (
		<Root location={location}>
			<main className={`${css(bs.main)}`}>
				<div className={`${css(bs.container)}`}>{renderBlogLinks(data)}</div>
			</main>
		</Root>
	);
};
const renderBlogLinks = (data) =>
	data.allMarkdownRemark.edges.map(({ node }) => (
		<Link to={node.frontmatter.path} key={node.id} className={`${css(bs.blogLink)} normal_font`}>
			...
			<div className={`${css(bs.linkInfo)}`}>
				...
				<span className={`${css(bs.linkDate)}`}>{node.frontmatter.date}</span>
			</div>
		</Link>
	));
